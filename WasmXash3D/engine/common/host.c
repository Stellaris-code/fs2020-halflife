...
void Host_Frame( float time )
{
	...
	// Print frame statistics on every amount of frames
	if ((host.framecount % 4) == 0 && false)
	{
		Con_Printf("Input: %fs, clientbegin: %fs, commands: %fs, server: %fs, client: %fs, total: %fs\n",
			host.inputframetime, host.clientbegintime, host.getcmdtime, host.svframetime, host.clframetime,
			host.pureframetime);
	}
...
//	Con_Reportf( "Sys_LoadLibrary: Loading xash.dll - ok\n" );
	...
	Con_Printf("Platform Ininitaized\n");
	...
	*/
	...
	Con_Printf("Before FS init\n");

	FS_Init();

	Con_Printf("FS Ininitaized\n");
