...
void Sys_InitLog( void )
{
	...
	// WASM
	if(1 || (Sys_CheckParm( "-log" ) && host.allow_console != 0 ))
	{
		s_ld.log_active = true;
		Q_strncpy( s_ld.log_path, "\\work\\engine.log", sizeof( s_ld.log_path ));
	}
...
#if (XASH_NSWITCH && NSWITCH_DEBUG) || 1
	// just spew it to stderr normally in debug mode
	static char print_buf[4096];
	sprintf(print_buf, "Xash3D: %s %s", logtime, buf);
	write(2, print_buf, 4096);
	//fprintf( stderr, "Xash3D: %s %s", logtime, buf );
