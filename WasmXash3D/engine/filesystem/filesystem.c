...
void listdirectory( stringlist_t *list, const char *path_in )
{
#if 1
	// Preopen 3 : '.'
	// Preopen 4 : 'work/'
	
	//fprintf(stderr, "stderr attempt\n");
	//Con_Printf("in listdirectory, attempting to open %s\n", path);
	...
	if (!(dir = opendir(path)))
	{
		//Con_Printf("Couldn't open dir %s\n", path);
		return;
	}
	...
	while ((entry = readdir(dir)))
	{
		...
		static char entry_buf[256];
		strcpy(entry_buf, "./");
		strcat(entry_buf, entry->d_name);
		stringlistappend(list, entry->d_name);

		//Con_Printf("Found dir %s\n", entry_buf);
	}
	closedir(dir);

	//Con_Printf("Dir closed", dir, list);
#endif
}
...
void FS_LoadGameInfo( const char *rootfolder )
{
	...
	Con_Reportf("Num games: %d\n", FI.numgames);
	// validate gamedir
	for( i = 0; i < FI.numgames; i++ )
	{
		Con_Reportf("Looking for %s : %s\n", FI.games[i]->gamefolder, fs_gamedir);
...
qboolean FS_InitStdio( qboolean caseinsensitive, const char *rootdir, const char *basedir, const char *gamedir, const char *rodir )
{
	...
	if( COM_CheckStringEmpty( fs_rodir ))
	{
		...
		{
			static char roPath[MAX_SYSPATH];
			...
			{
				static char rwPath[MAX_SYSPATH];
	...
	Con_Printf("Post listdirectory\n");

	for( i = 0; i < dirs.numstrings; i++ )
	{
		Con_Printf("Dir %s found\n", dirs.strings[i]);
	...
	for( i = 0; i < dirs.numstrings; i++ )
	{
		Con_Reportf("Candidate game is %s\n", dirs.strings[i]);
		...
		Con_Reportf("Candidate game %s is good\n", dirs.strings[i]);
...
====================
*/
int FS_SysFileTime( const char *path )
{
...
====================
*/
file_t *FS_SysOpen( const char *path, const char *mode )
{
	file_t	*file;
	int	mod, opt;
	uint	ind;
...
file_t *FS_OpenHandle( const char *path, int handle, fs_offset_t offset, fs_offset_t len )
{

	static char syspath[256];
	strcpy(syspath, "\\work\\");
	strcat(syspath, path);
...
==================
*/
qboolean FS_SysFileExists( const char *path_in )
{
	static char path[256];
	strcpy(path, "\\work\\");
	strcat(path, path_in);
...
#else
	__wasi_filestat_t wasi_stat;
	int ret = __wasi_path_filestat_get(4, 0, path, &wasi_stat);
	return ret == 0 && wasi_stat.filetype == __WASI_FILETYPE_REGULAR_FILE;
#endif
}
...
==================
*/
qboolean FS_SysFolderExists( const char *path_in )
{
	static char path[256];
	strcpy(path, "\\work\\");
	strcat(path, path_in);

	//Con_Reportf("Does %s exist?\n", path);
	// Cheat, bcz stat doesnt seem to work
	// It does!!!
	...
	__wasi_filestat_t wasi_stat;
	int ret = __wasi_path_filestat_get(4, 0, path, &wasi_stat);
	Con_Reportf("Ret for folder %s is %d\n", path, ret);
	return ret == 0 && wasi_stat.filetype == __WASI_FILETYPE_DIRECTORY;
#endif
}
...
==============
*/
qboolean FS_SysFileOrFolderExists( const char *path_in )
{
	return (FS_SysFolderExists(path_in) || FS_SysFileExists(path_in));
...
// WASM: lets try not to link chdir
/*
==================
...
#endif
#endif 
	Sys_Error("Attempted to change directory");
	return 0;
}
