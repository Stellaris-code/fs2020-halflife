...
static  pixel_t screenmap[256*256];
...
void R_BuildScreenMap( void )
{
	...
	vid.inv_screen = calloc(256 * 256 * 256, sizeof(pixel_t));
...
void R_AllocScreen( void )
{
	...
	gpGlobals->width = hl_fb_width;
	gpGlobals->height = hl_fb_height;
...
//	gEngfuncs.Con_Printf("blit begin\n");
	...
	for (v = 0; v < vid.height; v++)
	{
		...
		{
			unsigned int s = vid.rbg565toRGBA[vid.buffer[start + u] & 0xFFFF];
			pbuf[dstart + u] = s;
		}
	}

	double after = Sys_DoubleTime();
	static int frame = 0;
	//if (((frame++) % 16) == 0)
	//	gEngfuncs.Con_Printf("Blit time: %f / %f = %f\n", befur, after, (after - befur)*1000.0);
